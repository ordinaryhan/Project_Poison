...
public class PlayerMovement : MonoBehaviour {
    ...
    private EnemyMove[] EnemyScript = null;
    int EnemyIndex = 0;
    ...
    void Awake() {
        ...
        health = maxHealth;
        EnemyScript = new EnemyMove[Enemy.Length];
        for(int i = 0; i < EnemyScript.Length; i++)
        {
            EnemyScript[i] = Enemy[i].GetComponent<EnemyMove>();
        }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        //적과 충돌
         ...
         {
            if (ScoreUpdate.instance.isItem)
            {
                if(!isEnemyIndex)
                {
                    isEnemyIndex = true;
                    EnemyIndex = other.GetComponent<EnemyMove>().PathCenterIndex;
                }
    ...
    IEnumerator Itemact()
    {
        ScoreUpdate.instance.isItem = true;
        ...
        float keeptime = ScoreUpdate.instance.accumulate * 1.5f;
        ScoreUpdate.instance.ScoreReset();
        ...
        ScoreUpdate.instance.isItem = false;
